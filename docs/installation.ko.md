...
1. 프로젝트 이름
2. 프레임워크 (Next.js App Router / Pages Router / React)
3. 팀 규모 (Solo / Small / Medium / Large)
4. 아키텍처 패턴 (팀 규모에 따라 기본값 제안)
5. 라이브러리 (react-hook-form, zod, TanStack Query, shadcn/ui, Tailwind 등)
6. Feature 목록
7. 설치 확인
...
```
.claude/
  agents/          전문 에이전트 (init, spec-writer, planner, executor,
...
```bash
npx nextjs-claude-code@latest                       # 인터랙티브 설치
