...
- **큐레이션 스킬** — [skills.sh](https://skills.sh) (Claude Code 커뮤니티 스킬 레지스트리)에서 설치 시 자동 업데이트, 번들 폴백. React, Next.js, UI/UX, 테스팅, 라이브러리별 best practices 포함
...
| 명령어 | 설명 |
|--------|------|
| `/review [이름]` | 스펙 준수 + 코드 품질 리뷰. TEST_STRATEGY.md / LOG_STRATEGY.md / SECURITY_STRATEGY.md 존재 시 tester, log-auditor, security-reviewer 조건부 실행. |
...
| 명령어 | 설명 |
|--------|------|
| `/cicd` | CI/CD 파이프라인 설정. 플랫폼별 가이드 사용. spec/CICD.md 생성. |

### 개발 유틸리티

| 명령어 | 설명 |
|--------|------|
| `/init` | 기존 코드베이스 분석 + spec 문서 초안 생성. 설치 후 1회 실행. |
...
| `browser-tester` | 브라우저 테스트 + Figma 비교 | No | No (읽기 전용) |
...
| `cicd-builder` | CI/CD 파이프라인 생성 | Yes | Yes (CICD.md) |
