...
- **TDD 기본** — MSW API 목킹과 함께 Test-Driven Development가 기본 — 테스트 먼저, 구현은 그 다음
- **Hook profiles** — `NCC_HOOK_PROFILE`로 훅 강도 조절: `minimal` (보안만), `standard` (기본), `strict` (deprecation guard, comment checker, todo enforcer 추가)
- **Wave 실행** — 태스크를 의존성 기반 wave로 그룹화하여 병렬 디스패치, 빌드 시간 단축
- **Path-specific rules** — `.claude/rules/`에 `paths` 프론트매터로 매칭 파일에서만 로드되는 코딩 패턴 (App Router, DB, UI)
- **Context 최적화** — SessionStart repo profiler, compact recovery, artifact size limits
...
| 명령어 | 설명 |
|--------|------|
| `/review [이름]` | 전체 feature 단위 스펙 준수 + 코드 품질 리뷰. TEST/LOG/SECURITY_STRATEGY.md 존재 시 조건부 에이전트 실행. |
| `/loop [이름]` | 리뷰 → 수정 → 재검증 반복, 모든 REQ 통과까지 (최대 5회). |
| `/qa` | Playwright E2E 테스트, visual regression, 접근성 감사. `--visual` 스크린샷 비교. `--a11y` axe-core 감사. |
| `/test [이름]` | TEST_STRATEGY.md 기반 테스트 실행. `--browser`로 시각 테스트 + Figma 비교. `--setup`으로 설정. |
| `/log [이름]` | 로깅 관행 감사. `--audit`로 프로젝트 전체 스캔. `--setup`으로 설정. |
| `/security [이름]` | 보안 감사 (OWASP Top 10). `--audit`로 전체 스캔. `--diff`로 변경 파일만. `--setup`으로 설정. |
...
| 명령어 | 설명 |
|--------|------|
| `/issue-reporter "..."` | NCC GitHub 리포에 버그 리포트 또는 기능 요청을 제출. 모든 프로젝트/개인 정보를 자동 제거(sanitize) 후 사용자 확인을 거쳐 제출. |
...
| 명령어 | 설명 |
|--------|------|
| `/ncc-upgrade` | Claude Code 내에서 NCC 업그레이드. 플러그인/npx 설치 자동 감지. |
| `npx nextjs-claude-code upgrade` | 터미널에서 NCC 업그레이드 (npx 설치 전용). |

**업데이트 대상:** agents, hooks, scripts, rules, skills, CLAUDE.md NCC 블록, settings.json (병합).
**보존 대상:** `spec/feature/`, `spec/STATE.md`, CLAUDE.md 사용자 콘텐츠, `skills-lock.json`.

- **플러그인 모드** (`CLAUDE_PLUGIN_ROOT` 설정됨): git 원격에서 최신 pull, on-demand 스킬 업데이트.
- **npx 모드** (프로젝트에 파일 복사됨): npm에서 최신 패키지 다운로드, 시스템 파일 재설치, 스킬 업데이트.

업그레이드 후 최신 버전의 changelog가 터미널에 표시됩니다.
...
| `lead-engineer` | fresh-context subagent로 구현 오케스트레이션 (솔로 또는 팀 리더). Wave 기반 병렬 디스패치 지원. | No (오케스트레이터) | 부분 (STATE, history) |
...
| Agent | 역할 | 모델 | 코드 수정 |
|-------|------|:----:|:---------:|
| `task-executor` | [lead] 도메인 태스크 구현 (타입, 유틸, 훅, API, 서버 액션) | sonnet | Yes |
| `task-spec-reviewer` | 태스크별 스펙 준수 + 코드 품질 리뷰 | haiku | No (읽기 전용) |
| `performance-optimizer` | Core Web Vitals 진단 및 렌더링 전략 | sonnet | No (읽기 전용) |
...
| `issue-reporter` | NCC 버그/기능 요청을 GitHub에 제출 (프로젝트 데이터 자동 제거) | No | No |
...
`NCC_HOOK_PROFILE` 환경변수로 훅 강도 조절 (`.claude/settings.json`):

| Profile | 활성화 훅 |
|---------|----------|
| `minimal` | security-guard만 |
| `standard` (기본) | security-guard, validate-post-write, advisory-post-write, repo-profiler, compact-recovery |
| `strict` | standard 전체 + deprecation-guard, comment-checker, todo-enforcer |

### 추가 안전 장치

- **스펙 검증**: PostToolUse 훅이 잘못된 형식의 spec.md, design.md 작성을 차단
- **Deprecation guard** (strict): App Router에서 deprecated 패턴(getStaticProps, next/legacy/image 등) 차단
- **Comment checker** (strict): AI 생성 주석 밀도 30% 초과 시 경고
- **Todo enforcer** (strict): 미완료 PLAN.md 태스크, TODO/FIXME 경고
- **Repo profiler**: SessionStart 훅으로 package.json, tsconfig 스캔하여 컨텍스트 프라이밍
- **Compact recovery**: 컨텍스트 압축 후 활성 feature 상태 재주입
- **Artifact size limits**: spec 문서가 권장 줄 수 초과 시 경고 (`spec/rules/_artifact-limits.md`)
- **스펙 반영**: 코드 변경으로 새 export/route 추가 시 스펙 업데이트 리마인드
- **계획 노후화 검사**: spec.md가 PLAN.md 생성 이후 수정되었으면 경고
- **브랜치 전략 인식**: `/commit`과 `/pr`이 브랜치 전략 자동 감지, 커밋 컨벤션 적용
- **조건부 리뷰 에이전트**: 전략 파일 존재 시만 tester, log-auditor, security-reviewer가 `/review`에 참여
- **체인지로그 자동화**: `/pr`이 CHANGELOG.md 자동 업데이트 및 버전 범프 (Semantic Versioning)
...
| 훅 에러가 매 파일 작성마다 발생 | `.claude/settings.json` env에서 `NCC_HOOK_PROFILE=minimal`로 설정 |
| Deprecation guard가 정상 코드 차단 | `NCC_HOOK_PROFILE=standard`로 strict 훅 비활성화, 또는 `deprecation-rules.json` 수정 |
...
NCC는 Claude Code 생태계에서 검증된 최선의 패턴을 연구하고, 이를 하나의 일관된 워크플로우로 통합하여 만들어졌습니다. Wave 실행, 컨텍스트 엔지니어링, 에이전트 오케스트레이션, 토큰 최적화, 검증 루프, 스펙 기반 계획 등 핵심 기능은 [GSD](https://github.com/gsd-build/get-shit-done), [gstack](https://github.com/garrytan/gstack), [Everything Claude Code](https://github.com/affaan-m/everything-claude-code), [Oh My OpenAgent](https://github.com/code-yeongyu/oh-my-openagent), [Superpowers](https://github.com/obra/superpowers), [Vercel Plugin](https://github.com/vercel/vercel-plugin), [Spec Kit](https://github.com/github/spec-kit), [OpenSpec](https://github.com/Fission-AI/OpenSpec)의 인사이트를 반영하여 개선되었습니다.
